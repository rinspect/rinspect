 ...
 * http://www.cprover.org/wmm/esop13/x86_litmus//mix044.litmus
 *

X86 mix044
"Fre PodWW Wse PodWR Fre PodWW Wse PodWW Rfi PodRR"
Cycle=Rfi PodRR Fre PodWW Wse PodWR Fre PodWW Wse PodWW
Relax=[Rfi,PodRR] PodWR
Safe=Fre Wse PodWW
{
}
 P0         | P1          | P2         | P3          ;
 MOV [b],$1 | MOV [x],$2  | MOV [y],$1 | MOV [z],$2  ;
 MOV [x],$1 | MOV EAX,[y] | MOV [z],$1 | MOV [a],$1  ;
            |             |            | MOV EAX,[a] ;
            |             |            | MOV EBX,[b] ;
exists
(x=2 /\ z=2 /\ 1:EAX=0 /\ 3:EAX=1 /\ 3:EBX=0)
