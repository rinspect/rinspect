 ...
 * http://www.cprover.org/wmm/esop13/x86_litmus//mix040.litmus
 *

X86 mix040
"Fre PodWW Wse PodWW Wse PodWR Fre PodWW Rfi PodRR"
Cycle=Rfi PodRR Fre PodWW Wse PodWW Wse PodWR Fre PodWW
Relax=[Rfi,PodRR] PodWR
Safe=Fre Wse PodWW
{
}
 P0         | P1         | P2          | P3          ;
 MOV [b],$1 | MOV [x],$2 | MOV [y],$2  | MOV [z],$1  ;
 MOV [x],$1 | MOV [y],$1 | MOV EAX,[z] | MOV [a],$1  ;
            |            |             | MOV EAX,[a] ;
            |            |             | MOV EBX,[b] ;
exists
(x=2 /\ y=2 /\ 2:EAX=0 /\ 3:EAX=1 /\ 3:EBX=0)
...
int main(void) 
{
  ...
  //printf("x=%d,y=%d, cond2=%d, cond3=%d",x,y,cond2,cond3);
