 ...
 * http://www.cprover.org/wmm/esop13/x86_litmus//mix005.litmus
 *

 X86 mix005
 "Fre PodWW Wse PodWR Fre Rfi PodRR Fre Rfi PodRR"
 Cycle=Rfi PodRR Fre Rfi PodRR Fre PodWW Wse PodWR Fre
 ...
 MOV [x],$1 | MOV EAX,[y] | MOV EAX,[y] | MOV EAX,[z] ;
            |             | MOV EBX,[z] | MOV EBX,[a] ;
 exists
 (x=2 /\ 1:EAX=0 /\ 2:EAX=1 /\ 2:EBX=0 /\ 3:EAX=1 /\ 3:EBX=0)
...
int main(void) 
{
  ...
  // assert( !(x==2 && cond1 && cond2 && cond3 && cond0) );
  //printf("x is ---------%d/n ", x);
  if ( x==2 && cond1 && cond2 && cond3 && cond0 ) {
