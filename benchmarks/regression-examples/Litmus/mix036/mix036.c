 ...
 * http://www.cprover.org/wmm/esop13/x86_litmus//mix036.litmus
 *

X86 mix036
"Fre PodWR Fre PodWR Fre PodWR Fre PodWW Rfi PodRR"
Cycle=Rfi PodRR Fre PodWR Fre PodWR Fre PodWR Fre PodWW
Relax=[Rfi,PodRR] PodWR
Safe=Fre PodWW
{
}
 P0          | P1          | P2          | P3          ;
 MOV [b],$1  | MOV [x],$1  | MOV [y],$1  | MOV [z],$1  ;
 MOV EAX,[x] | MOV EAX,[y] | MOV EAX,[z] | MOV [a],$1  ;
             |             |             | MOV EAX,[a] ;
             |             |             | MOV EBX,[b] ;
exists
(0:EAX=0 /\ 1:EAX=0 /\ 2:EAX=0 /\ 3:EAX=1 /\ 3:EBX=0)
