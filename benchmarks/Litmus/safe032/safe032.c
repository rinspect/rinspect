 ...
 * http://www.cprover.org/wmm/esop13/x86_litmus//safe032.litmus
 *

X86 safe032
"Wse MFencedWR Fre MFencesWR Fre PodWW"
...
Safe=Fre Wse PodWW MFencesWR MFencedWR
{
 ...
 P0          | P1          | P2         ;
 MOV [y],$2  | MOV [x],$1  | MOV [x],$2 ;
 MFENCE      | MFENCE      | MOV [y],$1 ;
 MOV EAX,[x] | MOV EAX,[x] |            ;
exists
(x=2 /\ y=2 /\ 0:EAX=0 /\ 1:EAX=1)
...
int main(void) 
{
  ...
  //assert( ! (x==2 && y==2 && cond0 && cond1) );
  if ( x==2 && y==2 && cond0 && cond1 ) {
