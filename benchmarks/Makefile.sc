...
check-auto-race :
	...
	do \
	  ...
	  then \
		...
		if [ -f ./target ] ; \
	   	then \
		  echo $(INSPECT)   --max-runs 50000 ./target > result.autorace.raw ; \
		  $(INSPECT)  --max-runs 50000  ./target >> result.autorace.raw 2> result.stderr ; \
		...
		if diff result.autorace.filtered \
		  $(FULLEXAMPLEPATH)/$$example/check_result_race_sc > result.autorace.differences;\
		then \
		  echo "	Passed" ; \
